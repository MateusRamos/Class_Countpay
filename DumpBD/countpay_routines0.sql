...
--
/*!50003 DROP PROCEDURE IF EXISTS `coleta_dados_meses` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `coleta_dados_meses`(
    pid_usuario INT,
    pdata_inicial DATE,
    pdata_final DATE
)
BEGIN
    ...
    SELECT sum(valor)'receita' FROM lancamento WHERE id_usuario = pid_usuario AND tipo_lancamento LIKE '%Receita%' AND data_lancamento BETWEEN pdata_inicial AND pdata_final;
    SELECT sum(valor)'despesa' FROM lancamento WHERE id_usuario = pid_usuario AND tipo_lancamento LIKE '%Despesa%' AND data_lancamento BETWEEN pdata_inicial AND pdata_final;
    SELECT sum(valor)'fatura' FROM lancamento WHERE id_usuario = pid_usuario AND tipo_lancamento LIKE '%Despesa%' AND id_cartao > 0 AND data_lancamento BETWEEN pdata_inicial AND pdata_final;
...
BEGIN
    ...
    SELECT tipo_lancamento, valor, id_cartao FROM lancamento WHERE id_usuario = pid_usuario AND data_lancamento BETWEEN pdata_inicial AND pdata_final;
